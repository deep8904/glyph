...
export default async function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  return (
    <Shell breadcrumb={[{ label: name, href: `/dev/${username}` }, { label: 'Followers' }]}>
      <div className="mx-auto w-full max-w-2xl">
        <h1 className="text-h1 font-semibold text-fg">{entries.length} {entries.length === 1 ? 'follower' : 'followers'}</h1>
        {entries.length === 0 ? (
          <EmptyState className="mt-6" kind="first-use" title="No followers yet" description={`Nobody follows ${name} yet.`} />
        ) : (
