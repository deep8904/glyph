    ...
    <MotionConfig transition={springTransition}>
      <motion.li layout>
        <motion.div
          ...
          className="overflow-hidden border-solid border-gray-100 bg-white will-change-transform"
          style={{
            borderTopWidth,
            borderBottomWidth,
            borderLeftWidth,
        ...
        >
          ...
          >
            ...
            <motion.div animate={{ rotate: isOpen ? 180 : 0 }}>
              <ChevronDown className="size-5 text-gray-400 md:size-5" />
            </motion.div>
          </button>
          ...
          >
            <div ref={ref}>
              <div className="px-5 pb-5 text-xs font-normal text-gray-500 md:text-sm leading-relaxed">
                {item.content}
              </div>
            </div>
          </motion.div>
...
export const AccordionApp: FC<AccordionProps> = ({ items }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center transition-colors duration-500">
      <ul className="w-full">
