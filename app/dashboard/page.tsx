...
export default function DashboardPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      setProfile({
        name: meta.full_name || meta.name || meta.user_name || user.email?.split('@')[0] || 'Developer',
        email: user.email ?? '',
      })
    })
  }, [supabase])
  ...
  const initial = (profile.name || 'D').charAt(0).toUpperCase()
            ...
            <div className="px-4 py-4 border-t border-gray-100">
              <div className="flex items-center gap-3 px-2 py-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-400 font-mono truncate">{profile.email}</p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 px-6 md:px-10 py-8 space-y-8 overflow-y-auto">
              ...
              <div className="relative overflow-hidden rounded-3xl bg-gray-900 text-white p-8 md:p-12">
                ...
                <div className="relative z-10 max-w-xl">
                  <h1 className="text-2xl md:text-3xl font-light tracking-tight text-white mb-3">Welcome to Glyph.</h1>
                  <p className="text-sm md:text-base text-gray-400 leading-relaxed mb-8">
                    Your developer profile is ready. Start by adding your skills, engine, and current project.
                  </p>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <button className="inline-flex items-center justify-center rounded-full bg-indigo-600 px-6 py-3 text-sm font-medium text-white hover:bg-indigo-700 transition-all duration-300 shadow-lg shadow-indigo-600/20">
                      Set Up Profile
                    </button>
